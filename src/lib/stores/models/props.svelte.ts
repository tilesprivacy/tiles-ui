...
export class ModelPropsManager {
	 ...
	 */
	private cache = new TTLCache<string, ApiServerProps>({
	 ...
	 */
	async fetchModelProps(modelId: string): Promise<ApiServerProps | null> {
	...
	getModelProps(modelId: string): ApiServerProps | null {
	...
	private buildModalities(
		modalities: NonNullable<ApiServerProps['modalities']>
	): ModelModalities {
		return {
